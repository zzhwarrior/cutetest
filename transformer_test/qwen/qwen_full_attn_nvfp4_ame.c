...
// -------- AME wrapper tile geometry --------
#define AME_TILE_M           128
#define AME_TILE_N           128
#define AME_TILE_K_NVFP4     128       // 1 RV of NVFP4 @ CUTE_4Tops_128SCP
#define AME_TILE_K_BF16      32        // 1 RV of BF16 @ same config
#define AME_SCALE_ROW_BYTES  (AME_TILE_K_NVFP4 / NVFP4_BLOCK)   // 8

// Staging in TCM for AME→CPU handoff on fuse-op paths (128*128*fp32 = 64 KiB).
// Placed in the top way of the TCM aperture — Qwen weights/activations live
// in DRAM so TCM is otherwise unused here.
#define AME_STAGING_ADDR         0x81100000UL
#define AME_STAGING_STRIDE_BYTES ((uint64_t)(AME_TILE_N * sizeof(uint32_t)))
...
// -------- Step slicing for verilator (default 11 = full layer) --------
// Compile with -DRUN_UPTO_STEP=N to only execute steps 1..N.
//   1: RMSnorm+quant             2: +q_proj           3: +k_proj
//   4: +v_proj                   5: +gate_proj        6: +qk_norm+partrope
//   7: +score+softmax            8: +attn             9: +sigmoid_hadamard
//   10:+gated_attn_quant         11:+o_proj+resadd (full layer)
#ifndef RUN_UPTO_STEP
#define RUN_UPTO_STEP 11
#endif

// -------- Dummy quantization (skip the CPU-side FP4 encode) --------
// Reason: FP4 encode is scalar (8-level if-else stair × 16 elements/block × 32K blocks).
// It dominates CPU time in step 1 (~1.6M cycles) yet is NOT under test — we're
// measuring AME (tensor unit) cycles and correctness. Under DUMMY_QUANT=1 we
// memset the FP4 tensor to 0x22 (two FP4=1.0 codes per byte) and the scale to
// 0x38 (E4M3 encoding of 1.0). Then every GEMM's expected output is a constant:
// C[m][n] = sum_{k=0..K-1}(1.0 * 1.0 * 1.0 * 1.0) = K, easy to sanity-check.
// Set to 0 to run the real quantizer (needed only for numerical debug against CUTE).
#ifndef DUMMY_QUANT
#define DUMMY_QUANT 1
#endif
...
{
#if DUMMY_QUANT
    (void)in; (void)in_stride_bytes;
    // fp4=0x22 → two FP4 code 2 (1.0) per byte; scale=0x38 → E4M3 1.0.
    for (int r = 0; r < rows; r++) {
        uint8_t *row_fp4   = fp4_out   + r * fp4_stride_bytes;
        uint8_t *row_scale = scale_out + r * scale_stride_bytes;
        for (int j = 0; j < cols / 2; j++)             row_fp4[j]   = 0x22;
        for (int j = 0; j < cols / NVFP4_BLOCK; j++)   row_scale[j] = 0x38;
    }
    return;
#endif
...
//
// NOTE: GCC 15.1 miscompiles this at -O2/-O3 (see G.4 debug in
// ame-nvfp4-multitile-hang-and-k-semantics.md §8). Pin to O0 locally; softmax
// is O(N²) and <10% of layer time, so the slowdown is acceptable.
...
// ============================================================================
// AME wrappers — replace matmul_cute_* in the layer orchestrator.
// Signature-compatible with the CUTE versions so call sites transfer 1:1.
// ============================================================================

// Scale layout adaptation: qwen stores scale as row-major [M][K/16] but AME's
// ScaleLoader reads TILE_M×SCALE_ROW_BYTES contiguously (see G.5 discussion
// in ame-qwen-roadmap.md). Shuffle into tile-major [K_TILES][M][TILE_K/16]
// per matmul call. Two scratch buffers (A + B) sized for the largest scale
// in Qwen — proj_q_w scale is HIDDEN × HIDDEN/16 = 1 MiB. Static DRAM.
#define AME_SCALE_SCRATCH_MAX (HIDDEN * HIDDEN / NVFP4_BLOCK)   // 1 MiB
static uint8_t ame_scale_scratch_a[AME_SCALE_SCRATCH_MAX] __attribute__((aligned(64)));
static uint8_t ame_scale_scratch_b[AME_SCALE_SCRATCH_MAX] __attribute__((aligned(64)));

// dst layout:   [TILES_K][DIM_M][TILE_K/16]
// src layout:   [DIM_M][DIM_K/16], src_stride = DIM_K/16 (bytes per row)
static void ame_scale_shuffle_row2tile(uint8_t *dst, const uint8_t *src,
                                       size_t DIM_M, size_t DIM_K)
{
    const size_t SRC_ROW_BYTES = DIM_K / NVFP4_BLOCK;
    const size_t TILES_K       = DIM_K / AME_TILE_K_NVFP4;
    for (size_t kt = 0; kt < TILES_K; kt++) {
        for (size_t m = 0; m < DIM_M; m++) {
            memcpy(dst + kt * DIM_M * AME_SCALE_ROW_BYTES + m * AME_SCALE_ROW_BYTES,
                   src + m * SRC_ROW_BYTES + kt * AME_SCALE_ROW_BYTES,
                   AME_SCALE_ROW_BYTES);
        }
    }
}

// matmul_ame_nvfp4 — signature identical to matmul_cute_nvfp4.
...
{
    (void)element_wise_tensor;
    (void)scale_stride_A_bytes; (void)scale_stride_B_bytes;   // computed from DIM_K

    printf("[ame_nvfp4 enter] M=%zu N=%zu K=%zu after_op=%d\n",
           DIM_M, DIM_N, DIM_K, after_op);

    if ((DIM_M % AME_TILE_M) || (DIM_N % AME_TILE_N) || (DIM_K % AME_TILE_K_NVFP4)) {
        printf("matmul_ame_nvfp4: dims not multiple of tile\n");
        exit(1);
    }

    // Pick fuse-op fn and C element size (mirror qwen matmul_cute_nvfp4).
    after_op_fn fn = NULL;
    int C_elem_size = 4;
    switch (after_op) {
        case FUSE_NVFP4_QNORM_PARTROPE_BF16:
    ...
    // Shuffle scales row-major → tile-major.
    uint64_t t_shuf = mrdcycle();
    ame_scale_shuffle_row2tile(ame_scale_scratch_a, (const uint8_t *)A_scale, DIM_M, DIM_K);
    ame_scale_shuffle_row2tile(ame_scale_scratch_b, (const uint8_t *)B_scale, DIM_N, DIM_K);
    printf("[ame_nvfp4 shuf] done in %lu cycles, entering AME loop\n",
           (unsigned long)(mrdcycle() - t_shuf));

    const int TILES_M = DIM_M / AME_TILE_M;
    const int TILES_N = DIM_N / AME_TILE_N;
    const int TILES_K = DIM_K / AME_TILE_K_NVFP4;
    const size_t A_scale_tile_bytes = DIM_M * AME_SCALE_ROW_BYTES;
    const size_t B_scale_tile_bytes = DIM_N * AME_SCALE_ROW_BYTES;

    void *staging = (void *)AME_STAGING_ADDR;

    ame_settilem(AME_TILE_M);
    ame_settilen(AME_TILE_N);
    ame_settilek(AME_TILE_K_NVFP4);

    int acct = 0;
    for (int mt = 0; mt < TILES_M; mt++) {
        for (int nt = 0; nt < TILES_N; nt++) {
            void *c_tile = (uint8_t *)C
                + (size_t)mt * AME_TILE_M * stride_C_bytes
                + (size_t)nt * AME_TILE_N * C_elem_size;

            #define ISSUE_TILE(ACC)                                                       \
                ame_mzero(ACC);                                                           \
                for (int kt = 0; kt < TILES_K; kt++) {                                    \
                    uint64_t asb = (uint64_t)ame_scale_scratch_a                          \
                        + (uint64_t)kt * A_scale_tile_bytes                               \
                        + (uint64_t)mt * AME_TILE_M * AME_SCALE_ROW_BYTES;                \
                    uint64_t bsb = (uint64_t)ame_scale_scratch_b                          \
                        + (uint64_t)kt * B_scale_tile_bytes                               \
                        + (uint64_t)nt * AME_TILE_N * AME_SCALE_ROW_BYTES;                \
                    ame_mset_scalea(asb);                                                 \
                    ame_mset_scaleb(bsb);                                                 \
                    uint64_t ab = (uint64_t)A_fp4                                         \
                        + (uint64_t)mt * AME_TILE_M * stride_A_bytes                      \
                        + (uint64_t)kt * (AME_TILE_K_NVFP4 / 2);                          \
                    uint64_t bb = (uint64_t)B_fp4                                         \
                        + (uint64_t)nt * AME_TILE_N * stride_B_bytes                      \
                        + (uint64_t)kt * (AME_TILE_K_NVFP4 / 2);                          \
                    if ((kt & 1) == 0) {                                                  \
                        ame_mlae4(TR0, ab, stride_A_bytes);                               \
                        ame_mlbe4(TR2, bb, stride_B_bytes);                               \
                        ame_mfmacc_s_nvfp4(ACC, TR0, TR2);                                \
                    } else {                                                              \
                        ame_mlae4(TR1, ab, stride_A_bytes);                               \
                        ame_mlbe4(TR3, bb, stride_B_bytes);                               \
                        ame_mfmacc_s_nvfp4(ACC, TR1, TR3);                                \
                    }                                                                     \
                }                                                                         \
                ame_msce32(ACC, (uint64_t)staging, AME_STAGING_STRIDE_BYTES);
            if (acct == 0) { ISSUE_TILE(ACC0); acct = 1; }
            else           { ISSUE_TILE(ACC1); acct = 0; }
            #undef ISSUE_TILE

            ame_fence();
            fn(staging, c_tile, NULL, NULL,
               AME_TILE_M, AME_TILE_N,
               AME_STAGING_STRIDE_BYTES, stride_C_bytes, extra);
...
// matmul_ame_bf16 — signature identical to matmul_cute_bf16.
// after_op == NO_ACTIVATION: msce32 direct to C fp32.
// after_op == FUSE_MASKED_SOFTMAX_BF16CVRT: staging + fuse-op, C fp16 output.
//   DIM_N must be ≤ TILE_N (softmax reduces over full N per row).
static void matmul_ame_bf16(
    ...
    int after_op, void *extra)
{
    printf("[ame_bf16 enter] M=%zu N=%zu K=%zu after_op=%d\n",
           DIM_M, DIM_N, DIM_K, after_op);
    if ((DIM_M % AME_TILE_M) || (DIM_N % AME_TILE_N) || (DIM_K % AME_TILE_K_BF16)) {
        printf("matmul_ame_bf16: dims not aligned\n");
        exit(1);
    }

    int softmax_path = 0;
    if (after_op == FUSE_MASKED_SOFTMAX_BF16CVRT) {
        softmax_path = 1;
        if (DIM_N != AME_TILE_N) {
            printf("matmul_ame_bf16: softmax requires DIM_N==%d, got %zu\n",
                   AME_TILE_N, DIM_N);
            exit(1);
        }
    } else if (after_op != NO_ACTIVATION) {
    ...
    const int TILES_M = DIM_M / AME_TILE_M;
    const int TILES_N = DIM_N / AME_TILE_N;
    const int TILES_K = DIM_K / AME_TILE_K_BF16;
    const size_t TILE_K_BYTES = AME_TILE_K_BF16 * sizeof(uint16_t);
    void *staging = (void *)AME_STAGING_ADDR;
    int C_elem_size = softmax_path ? 2 : 4;

    ame_settilem(AME_TILE_M);
    ame_settilen(AME_TILE_N);
    ame_settilek(AME_TILE_K_BF16);
    ...
    for (int mt = 0; mt < TILES_M; mt++) {
        for (int nt = 0; nt < TILES_N; nt++) {
            ...
            uint64_t store_stride = softmax_path ? AME_STAGING_STRIDE_BYTES : stride_C_bytes;

            #define ISSUE_TILE_BF16(ACC)                                                  \
                ame_mzero(ACC);                                                           \
                for (int kt = 0; kt < TILES_K; kt++) {                                    \
                    uint64_t ab = (uint64_t)A_bf16                                        \
                        + (uint64_t)mt * AME_TILE_M * stride_A_bytes                      \
                        + (uint64_t)kt * TILE_K_BYTES;                                    \
                    uint64_t bb = (uint64_t)B_bf16                                        \
                        + (uint64_t)nt * AME_TILE_N * stride_B_bytes                      \
                        + (uint64_t)kt * TILE_K_BYTES;                                    \
                    if ((kt & 1) == 0) {                                                  \
                        ame_mlae16(TR0, ab, stride_A_bytes);                              \
                        ame_mlbe16(TR2, bb, stride_B_bytes);                              \
                        ame_mfmacc_s_bf16(ACC, TR0, TR2);                                 \
                    } else {                                                              \
                        ame_mlae16(TR1, ab, stride_A_bytes);                              \
                        ame_mlbe16(TR3, bb, stride_B_bytes);                              \
                        ame_mfmacc_s_bf16(ACC, TR1, TR3);                                 \
                    }                                                                     \
                }                                                                         \
                ame_msce32(ACC, store_dest, store_stride);
            if (acct == 0) { ISSUE_TILE_BF16(ACC0); acct = 1; }
            else           { ISSUE_TILE_BF16(ACC1); acct = 0; }
            #undef ISSUE_TILE_BF16

            if (softmax_path) {
                ame_fence();
                fuse_masked_softmax_bf16(staging, c_tile, NULL, NULL,
                                         AME_TILE_M, (int)DIM_N,
                                         AME_STAGING_STRIDE_BYTES, stride_C_bytes, extra);
                asm volatile ("fence rw, rw" ::: "memory");
            }
        }
...
static void qwen_full_attn_block(void)
{
    ...
    uint64_t t0 = t_layer_start;
    int rope_pos_base = 0;
    (void)rope_pos_base;

    printf("[step] RUN_UPTO_STEP=%d\n", RUN_UPTO_STEP);
    ...
    t0 = mrdcycle();
    matmul_ame_nvfp4(SEQ_LEN, HIDDEN, HIDDEN,
    ...
    t0 = mrdcycle();
    matmul_ame_nvfp4(SEQ_LEN, N_HEAD_KV * HEAD_DIM, HIDDEN,
    ...
    for (int h = 0; h < N_HEAD_Q; h++) {
        ...
        matmul_ame_bf16(SEQ_LEN, SEQ_LEN, HEAD_DIM,
                         A, B, C,
                         N_HEAD_Q  * HEAD_DIM * 2,
                         N_HEAD_KV * HEAD_DIM * 2,
                         SEQ_LEN * 2,
    ...
    for (int h = 0; h < N_HEAD_Q; h++) {
        ...
        matmul_ame_bf16(SEQ_LEN, HEAD_DIM, SEQ_LEN,
                         A, B, C,
                         SEQ_LEN * 2,
                         N_HEAD_KV * HEAD_DIM * 2,
                         HIDDEN * 4,
