...
//funct === 6    配置加速器，cfgData1 = (element_type[0~7bit]，bias_type[8~15bit]，...)
//funct === 7    配置加速器，cfgData1 = ScaleA的基地址（NVFP4/MXFP*用；无stride字段，硬件按element_type自推）
//funct === 8    配置加速器，cfgData1 = ScaleB的基地址（同上）
...
#define CUTEDataTypeMxfp8e4m3F32    7     //MXFP8 E4M3
#define CUTEDataTypeMxfp8e5m2F32    8     //MXFP8 E5M2  (CUTEParameters.scala:2114)
#define CUTEDataTypeNVFP4NVFP4F32   9     //NVFP4 * NVFP4 * FP32  (CUTEParameters.scala:2115)
#define CUTEDataTypeMxfp4F32        10    //MXFP4
...
#define CUTE_SCALEA_CONFIG_FUNCTOPS (CUTE_CONFIG_FUNCTOPS + 7)
#define CUTE_SCALEB_CONFIG_FUNCTOPS (CUTE_CONFIG_FUNCTOPS + 8)
...
// funct=7: ConfigScaleA — only base vaddr; stride derived by HW from element_type
void issue_cute_config_ScaleA(uint64_t ScaleA_Base_Vaddr)
{
    int result;
    YGJK_INS_RRR(result, ScaleA_Base_Vaddr, 0, CUTE_SCALEA_CONFIG_FUNCTOPS);
}

// funct=8: ConfigScaleB — analogous
void issue_cute_config_ScaleB(uint64_t ScaleB_Base_Vaddr)
{
    int result;
    YGJK_INS_RRR(result, ScaleB_Base_Vaddr, 0, CUTE_SCALEB_CONFIG_FUNCTOPS);
}
...
// NVFP4 variant: same as issue_cute_matmul_marco_inst but hard-codes element_type=9
// and configures ScaleA/ScaleB base vaddrs via funct=7/8.
// ScaleA layout: [M × K/16] E4M3 bytes, one per NVFP4 K-block along the reduce axis.
// ScaleB layout: [N × K/16] E4M3 bytes, same convention.
uint64_t issue_cute_matmul_marco_inst_nvfp4(
    uint64_t ATensor_Base_Addr, uint64_t ATensor_M_Stride,
    uint64_t BTensor_Base_Addr, uint64_t BTensor_M_Stride,
    uint64_t BiasTensor_Base_Addr, uint64_t BiasTensor_M_Stride,
    uint64_t CTensor_Base_Addr, uint64_t CTensor_M_Stride,
    uint64_t ScaleA_Base_Vaddr, uint64_t ScaleB_Base_Vaddr,
    uint64_t M, uint64_t N, uint64_t K,
    uint64_t bias_type, uint64_t transpose_result, uint64_t matmul_m_index)
{
    issue_cute_config_ATensor(ATensor_Base_Addr, ATensor_M_Stride);
    issue_cute_config_BTensor(BTensor_Base_Addr, BTensor_M_Stride);
    issue_cute_config_CTensor(BiasTensor_Base_Addr, BiasTensor_M_Stride);
    issue_cute_config_DTensor(CTensor_Base_Addr, CTensor_M_Stride);
    issue_cute_config_ScaleA(ScaleA_Base_Vaddr);
    issue_cute_config_ScaleB(ScaleB_Base_Vaddr);
    issue_cute_config_MNK_KERNALSTRIDE(M, N, K, 0);
    issue_cute_config_MatMul(CUTEDataTypeNVFP4NVFP4F32, bias_type, transpose_result, matmul_m_index);
    return issue_cute_marco_inst();
}
...
// Poll until the marco-inst with given task_id has completed, then dequeue.
// Mirrors llama3_1B.c:646 CUTE_TASK_END.
static inline void CUTE_TASK_END(uint64_t task_id)
{
    uint64_t finish_tag = 1ULL << task_id;
